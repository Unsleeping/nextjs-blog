...
const CTACard = ({ dictionary }: { dictionary: any }) => {
  ...
  // TODO: clear here and add pending state from useFormState
  const formAction = async (formData: FormData) => {
    "use server";
    try {
      const email = formData.get("email");
      const check = await directus.items("subscribers").createOne({
        email,
      });
      console.log(check);
  ...
  //     e.preventDefault();
  //     setIsHandling(true);
  //     const res = await directus.items("subscribers").createOne(
  //       {
  //         email,
  //       },
  //       undefined,
